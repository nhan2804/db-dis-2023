...
export class BaseService<T, S = T & Document> {
  ...
  async baseFindOne(id: string, errorCallback = null) {
    try {
  ...
  async baseUpdateOne(id: string, data: BaseModel, errorCallback = null) {
    try {
  ...
  async baseDeleteOnce(id: string, errorCallback = null) {
    try {
